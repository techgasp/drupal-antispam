...
class SpamMasterCleanUpController extends ControllerBase {
  ...
  public function spammastercleanupkeys() {
    ...
    if ($spammaster_license_status == 'VALID' || $spammaster_license_status == 'MALFUNCTION_1' || $spammaster_license_status == 'MALFUNCTION_2') {
      ...
      // Log message.
      \Drupal::logger('spammaster-cleanup')->notice('Spam Master: weekly cleanup successful run.');
      $spammaster_db_cleanup_insert = db_insert('spammaster_keys')->fields([
        'date' => $time,
        'spamkey' => 'spammaster-cleanup',
        'spamvalue' => 'Spam Master: weekly cleanup successful run.',
      ])->execute();
    }
    else {
      // Log message.
      \Drupal::logger('spammaster-cleanup')->notice('Spam Master: weekly cleanup did run, check your license status.');
      $spammaster_db_cleanup_insert = db_insert('spammaster_keys')->fields([
        'date' => $time,
        'spamkey' => 'spammaster-cleanup',
        'spamvalue' => 'Spam Master: weekly cleanup did run, check your license status.',
      ])->execute();
    }
